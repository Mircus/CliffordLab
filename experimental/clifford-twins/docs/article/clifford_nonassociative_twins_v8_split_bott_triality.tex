...
Active Volume, Split Twins, and Folded Triality}
...
This note develops that interpretation in four layers. First, it records the
standard twisted Boolean-cube description, where Clifford algebras and
nonassociative cousins differ by whether the associator is trivial or curved.
Second, it replaces the basis-level sign picture by an invariant Clifford-body
picture: one should not keep only the naked algebra, nor only a presentation
$\Cl(V,Q)$, but the graded body $(\mathcal C,\alpha,\widetilde{(\cdot)},\mathfrak v)$
with reversion and central volume line. Third, it works out the companion case
$\Cl(0,3)$, where the same active-volume step gives the split octonions. This
provides a second example and clarifies that the construction is controlled by
the sign of the volume square. Fourth, it connects the construction to
...
Finally, Bott periodicity suggests a stabilized version: $\Cl(8+8m,0)$ carries
the same $D_4$ triality cell with matrix coefficients. The contribution claimed
here is not a new construction of the octonions, but a geometric reframing of
the Cayley--Dickson step inside $\Cl(3,0)$, its split companion in $\Cl(0,3)$,
and the relation between folded, unfolded, and Bott-stabilized triality.
\end{abstract}
...
\section{A second worked example: $\Cl(0,3)$ and split octonions}

The construction is clearer if one also looks at the opposite definite
signature. Let
\[
\Cl(0,3)=\langle f_1,f_2,f_3\mid f_i^2=-1,\ f_if_j=-f_jf_i\rangle.
\]
The even subalgebra is again quaternionic:
\[
\Cl^+(0,3)=\langle 1,f_{23},f_{31},f_{12}\rangle\cong \mathbb H.
\]
Let
\[
J=f_1f_2f_3.
\]
Now the square of the pseudoscalar is different. In general,
\[
I^2=(-1)^{q+n(n-1)/2},
\qquad n=p+q.
\]
For $(p,q)=(3,0)$ one has $I^2=-1$, while for $(p,q)=(0,3)$ one has
\[
J^2=+1.
\]
The same invariant split exists:
\[
\Cl(0,3)=\Cl^+(0,3)\oplus \Cl^+(0,3)J
        \cong \mathbb H\oplus\mathbb HJ.
\]
The passive Clifford product treats $J$ as a central volume element with
positive square:
\[
(a+bJ)(c+dJ)=(ac+bd)+(ad+bc)J,
\qquad a,b,c,d\in\mathbb H.
\]
The active-volume twin again replaces the central crossing law by the
conjugating law
\[
\ell a=\bar a\ell,
\]
but now the new unit satisfies
\[
\ell^2=+1.
\]
Thus the corresponding Cayley--Dickson product is
\[
(a+b\ell)\star_+(c+d\ell)
=
(ac+\bar d\,b)+(da+b\bar c)\ell.
\]
This is the split-octonion product. Therefore the same construction gives
\[
\Cl(3,0)\quad\leadsto\quad \mathbb O,
\]
where the active volume unit has square $-1$, and
\[
\Cl(0,3)\quad\leadsto\quad \mathbb O_s,
\]
where the active volume unit has square $+1$.

This second example is useful because it shows that the proposal is not merely
``$\Cl(3,0)$ has the same dimension as $\mathbb O$.'' The mechanism is the
odd-dimensional volume split
\[
\Cl(p,q)=\Cl^+(p,q)\oplus\Cl^+(p,q)I
\]
together with the replacement of passive volume by active volume. The sign of
$I^2$ determines whether the resulting doubling is compact or split.

The low-dimensional cases may be summarized as follows.
\[
\begin{array}{c|c|c|c}
\text{Clifford body} & \text{even core }A=\Cl^+(p,q) & I^2 & \text{active-volume twin}\\
\hline
\Cl(3,0) & \mathbb H & -1 & \mathbb O\\
\Cl(0,3) & \mathbb H & +1 & \mathbb O_s\\
\Cl(2,1) & \mathbb H_s\cong M_2(\mathbb R) & +1 & \text{split-type }8\text{D twin}\\
\Cl(1,2) & \mathbb H_s\cong M_2(\mathbb R) & -1 & \text{split-type }8\text{D twin}
\end{array}
\]
Here $\mathbb H_s$ denotes the split quaternions. Only the first line gives the
classical division octonions. The second gives the split octonions. The mixed
signatures give analogous split-type active-volume doubles of the split
quaternionic even core. These examples make the later classification problem
more concrete: one should ask which signatures yield alternative, flexible, or
power-associative twins, rather than assume that all higher twins preserve the
special features of $\mathbb O$.
...
\section{Interpretive geometry: switching the role of volume}

The following discussion is an interpretive geometric reading of the associator, not a proposed physical model. It explains what the algebraic operation does to rotor data, and it should be read at that level.
...
\begin{quote}
Octonionic nonassociativity records where chirality reversal occurred in a
rotational process.
\end{quote}
...
This claim is not merely metaphorical. Let $R_1,R_2\in H$ be unit rotors. In
the passive Clifford product one has
\[
I(R_1R_2)=(R_1R_2)I=(IR_1)R_2,
\]
because $I$ is central. In the active product, however,
\[
I(R_1R_2)=\overline{R_1R_2}I=\bar R_2\bar R_1 I,
\]
while crossing the gate before the second rotor gives
\[
(IR_1)R_2=(\bar R_1I)R_2=\bar R_1\bar R_2I.
\]
Since quaternionic rotors need not commute,
\[
\bar R_1\bar R_2\neq \bar R_2\bar R_1
\]
in general. Hence
...
(IR_1)R_2\neq I(R_1R_2).
\]
The octonionic spinor-like element therefore remembers whether the frame has
crossed the handedness gate before or after a given rotation. This is the
calculation behind the slogan that active-volume spinors carry process memory.
...
\begin{quote}
Clifford spinors rotate frames; octonionic spinors remember whether a frame
crossed the handedness gate.
\end{quote}
...
\section{Bott-stabilized triality}

There is a second, more programmatic extension of the folded/unfolded picture.
It should be stated carefully. The exceptional triality of $\mathrm{Spin}(8)$
is a $D_4$ phenomenon. It does not repeat as an ordinary triality of
$\mathrm{Spin}(16)$, $\mathrm{Spin}(24)$, and so on. For example, in dimension
$16$ the vector representation has dimension $16$, while the half-spinor
representations have dimension $128$, so there is no democratic permutation of
three $16$-dimensional representation spaces analogous to $D_4$ triality.

What does repeat is the Clifford envelope. Real Bott periodicity gives
\[
\Cl(n+8,0)\cong \Cl(n,0)\otimes M_{16}(\mathbb R),
\]
and, in particular,
\[
\Cl(16,0)\cong \Cl(8,0)\otimes M_{16}(\mathbb R).
\]
Thus higher Clifford algebras do not contain new ordinary $D_4$ trialities;
they contain matrix-stabilized copies of the original unfolded triality cell.
In this sense,
\[
\Cl(8,0)=\text{unfolded triality over }\mathbb R,
\]
whereas
\[
\Cl(16,0)=\text{unfolded triality over }M_{16}(\mathbb R),
\]
and more generally
\[
\Cl(8+8m,0)
\cong
\Cl(8,0)\otimes M_{16^m}(\mathbb R).
\]
This may be called \emph{Bott-stabilized triality} or \emph{Morita triality}:
the same $D_4$ triality package is transported through the Bott-periodic matrix
factor.

The folded analogue is obtained by replacing the unfolded envelope
$\Cl(8,0)$ by the compact octonionic triality cell $\mathbb O$. Formally one
is led to matrix-coefficient octonionic spaces
\[
\mathbb O\otimes M_{16^m}(\mathbb R),
\]
not as new normed division algebras, but as folded triality cells with
Bott-stabilized coefficients. The original trilinear form
\[
t:V\times S_+\times S_-\to\mathbb R
\]
may then be replaced schematically by a matrix-coefficient form. For pure
tensors one expects a trace-paired expression of the type
\[
T_A(x\otimes A,\psi\otimes B,\phi\otimes C)
=
 t(x,\psi,\phi)\,\operatorname{Tr}(ABC),
\]
where
\[
A=M_{16^m}(\mathbb R).
\]
This formula is meant only as the simplest model of the idea: ordinary
triality form plus matrix coefficient trace. A fully invariant formulation
should be phrased in Morita-theoretic language, because the matrix algebra is
not changing the underlying $D_4$ triality; it is stabilizing its coefficient
category.

Thus the folded/unfolded diagram extends as follows:
\[
\begin{array}{c|c|c}
\text{level} & \text{unfolded Clifford envelope} & \text{folded octonionic cell}\\
\hline
0 & \Cl(8,0) & \mathbb O\\
1 & \Cl(16,0)\cong \Cl(8,0)\otimes M_{16}(\mathbb R) & \mathbb O\otimes M_{16}(\mathbb R)\\
2 & \Cl(24,0)\cong \Cl(8,0)\otimes M_{256}(\mathbb R) & \mathbb O\otimes M_{256}(\mathbb R)\\
\vdots & \vdots & \vdots
\end{array}
\]
The warning is essential: this is not a claim of new $\mathrm{Spin}(16)$ or
$\mathrm{Spin}(24)$ triality. It is the original $\mathrm{Spin}(8)$ triality
cell, unfolded or folded, with Bott-periodic matrix coefficients.
...
A nonassociative twin of a Clifford algebra $\Cl(p,q)$ may be understood
in two compatible senses. In the broad Boolean-cube sense it is a real
...
\assoc_F(a,b,c)\neq 1
\]
for some triples, while preserving as much as possible of the Clifford sign
geometry, grading, and quadratic signature. In the narrower odd-volume sense,
when $n$ is odd and
\[
\Cl(p,q)=A\oplus AI,
\qquad A=\Cl^+(p,q),
\]
one obtains a canonical active-volume candidate by replacing the passive law
$Ia=aI$ with the conjugating law $Ia=\widetilde aI$, with the sign of $I^2$
controlling whether the doubling is compact or split.
\end{definition}
...
spinorial side. The use of Clifford modules in real $K$-theory and the
periodicity of real Clifford algebras goes back to Atiyah--Bott--Shapiro
\cite{AtiyahBottShapiro}; the Bott-stabilized triality discussion here is only
a repackaging of the standard periodicity
$\Cl(n+8,0)\cong\Cl(n,0)\otimes M_{16}(\mathbb R)$ around the $D_4$
triality cell.
...
\addlinespace
Bott-stabilized triality & $\Cl(8,0)\otimes M_{16^m}(\mathbb R)$ or $\mathbb O\otimes M_{16^m}(\mathbb R)$ & Matrix/Morita coefficient stabilization & Same $D_4$ triality cell with matrix coefficients, not new $\mathrm{Spin}(16)$ triality & Programmatic extension proposed here \\
\addlinespace
...
\addlinespace
Volume-twin interpretation in this note & $\Cl^+(3,0)\oplus\Cl^+(3,0)I$ & Reversion on $\Cl^+(3,0)$ plus active pseudoscalar $I$ & Associator records where chirality reversal occurs & Proposed contribution: geometric reframing \\
\bottomrule
\end{tabular}%
}
\end{center}
...
This interpretation also gives a compact interpretive reading of the associator.
...
geometric explanation of how the octonionic product can be placed on the same
...
The dimension-$8$ cases $\Cl(3,0)\leadsto\mathbb O$ and
$\Cl(0,3)\leadsto\mathbb O_s$ are the prototypes. Higher dimensions should
not be expected to preserve all octonionic miracles. In particular, normed
division algebra behavior is exceptional. But the active-volume and
twisted-hypercube viewpoints give a uniform search space for Clifford-like
nonassociative cousins.

A more concrete computational program is:
\begin{enumerate}
    \item For odd $n=p+q$, implement the volume split
    \[
    \Cl(p,q)=\Cl^+(p,q)\oplus\Cl^+(p,q)I
    \]
    and the active-volume product
    \[
    \begin{aligned}
    (a+bI)\star(c+dI)
    &=(ac+I^2\,\widetilde d\,b)\\
    &\quad +(da+b\widetilde c)I.
    \end{aligned}
    \]
    \item Determine when the resulting twin is alternative, flexible, or
    power-associative.
    \item Compare the active-volume associator with the associator obtained
    from the $(\Ztwo)^n$ twisted-cube formulation.
    \item Identify which twins are merely matrix-coefficient/split variants
    and which, if any beyond dimension $8$, have genuinely new structure.
\end{enumerate}
...
\frac{F[a,b]F[a\oplus b,c]}
     {F[b,c]F[a,b\oplus c]}.
\]
This makes the theory ideal for a small Python package. Such a package
accompanies this article.

\subsection{Companion repository: \texttt{clifford-twins}}

A companion Python repository implements the constructions of this article
and provides a computational verification layer. The package is organized as
follows.

\medskip

\noindent\textbf{Core modules.}
...
\midrule
\texttt{quaternion.py} & Frozen-dataclass quaternion arithmetic: Hamilton
  product, conjugation, norm, inverse. \\
\texttt{cl3.py} & The shared 8D carrier $\mathcal C = H \oplus HI$, represented
  as a pair of quaternions. This class is product-agnostic: it implements only
  vector-space operations (addition, subtraction, scalar multiplication,
  Cayley--Dickson conjugation, norm). \\
\texttt{products.py} & The two multiplications on the same carrier:
  \texttt{clifford\_product} (passive volume, $Ia = aI$) and
  \texttt{octonion\_twin\_product} (active volume, $\ell a = \bar{a}\ell$).
  Also: \texttt{associator} and \texttt{commutator} as higher-order functions
  parameterized by product choice. \\
\texttt{basis.py} & The standard 8-element basis
  $\{1, i, j, k, \ell, i\ell, j\ell, k\ell\}$, corresponding to the
  $(\mathbb Z_2)^3$ grading. \\
\texttt{identities.py} & Checks for associativity and alternativity on the
  full basis: \texttt{is\_associative\_on\_basis},
  \texttt{is\_alternative\_on\_basis}, and \texttt{associator\_table}
  (enumerates all nonzero basis associators). \\
\texttt{triality.py} & The octonionic triality form
  $t(x,\psi,\phi) = \operatorname{Re}(x \cdot (\psi \cdot \phi))$, inner
  product, and real-part extraction. \\
\texttt{twisted\_cube.py} & The $(\mathbb Z_2)^n$ twisted-hypercube framework
  from Section~2: a \texttt{Twist} dataclass parameterized by a sign rule
  $F: G \times G \to \{\pm 1\}$, with methods for basis multiplication,
  associator factor $\Phi_F(a,b,c)$, and full associativity check. \\
\bottomrule
\end{tabular}
\end{center}
...
The central design decision is that the carrier
\texttt{Cl3Element} does \emph{not} carry a product. It is a pure
vector-space element. Multiplications are external functions that take two
elements and return a third. This mirrors the article's key point: one
vector space, two algebra structures (Section~8). The same Python object
\texttt{x} can be multiplied under either product:
\begin{verbatim}
    from clifford_twins.products import clifford_product, octonion_twin_product

    z_cl  = clifford_product(x, y)       # passive volume: Ia = aI
    z_oct = octonion_twin_product(x, y)   # active volume:  la = conj(a) l
\end{verbatim}

\noindent\textbf{What the tests verify.}
The test suite (23 tests at time of writing) verifies:
\begin{itemize}
    \item Clifford product is associative on all basis triples ($8^3 = 512$
    triples).
    \item Octonion twin product is \emph{not} associative but \emph{is}
    alternative on all basis pairs.
    \item The associator table has exactly the expected nonzero entries.
    \item The triality form satisfies the expected cyclic symmetry.
    \item The twisted-cube \texttt{Twist} class correctly distinguishes
    associative (Clifford) and nonassociative (octonionic) sign rules.
    \item Quaternion arithmetic: Hamilton product, conjugation, norm, inverse.
\end{itemize}

\noindent\textbf{Relation to article sections.}
Sections~2--4 (twisted hypercubes and associator factor) correspond to
\texttt{twisted\_cube.py}.
Sections~5--6 (invariant $\Cl(3,0)$ construction) correspond to
\texttt{cl3.py} and \texttt{products.py}.
Section~7 ($\Cl(0,3)$ and split octonions) is not yet implemented in the
repository; this is a natural next step.
Section~12 (triality) corresponds to \texttt{triality.py}.
The research program in Section~15 outlines the computational search that
\texttt{twisted\_cube.py} is designed to support.
...
The companion example $\Cl(0,3)$ shows that the same active-volume mechanism
is not confined to the compact octonions. Since its pseudoscalar has square
$+1$, the same construction produces the split octonions. Thus the sign of the
volume square controls the compact/split character of the twin.

The corresponding spinor picture is equally sharp once expressed as a
calculation: Clifford spinors are rotors in passively oriented three-space,
while octonionic spinor-like elements are rotors together with chirality-gated
rotors. Their nonassociativity records where the handedness gate occurs in a
rotational process.
...
cell in which folded triality naturally lives. Bott periodicity then stabilizes
this picture: $\Cl(8+8m,0)$ should be read not as a source of new ordinary
trialities, but as the original $D_4$ triality cell with matrix coefficients.
...
Topology 3, Supplement 1 (1964), 3--38.
DOI: \url{https://doi.org/10.1016/0040-9383(64)90003-5}.

\bibitem{ManogueSchray}
C. A. Manogue and J. Schray,
\emph{Octonionic representations of Clifford algebras and triality},
Foundations of Physics 26 (1996), 17--70.
Available as arXiv:hep-th/9407179.
